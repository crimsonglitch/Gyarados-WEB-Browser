...
from PyQt5.QtWidgets import (QApplication, QMainWindow, QWidget, QVBoxLayout, 
                            ...
                            QComboBox, QGroupBox, QSlider, QTextEdit)
...
class Config:
    ...
    def load(cls, profile_name="default"):
        ...
        config.load_history(profile_name)
        return config
    ...
    def save(self):
        ...
        self.save_history(self.CURRENT_PROFILE)
    ...
    def save_history(self, profile_name):
        try:
            ...
            with open(temp_file, 'w', encoding='utf-8') as f:
                json.dump(self.load_history(profile_name), f, ensure_ascii=False, indent=2)
...
class WebTab(QWidget):
    ...
    def setup_web_engine(self):
        ...
        # Favicon renk uyumu için CSS enjeksiyonu
...
class NavigationBar(QWidget):
    ...
    def show_history_manager(self):
        self.parent_window.show_history_manager()
...
class MainWindow(QMainWindow):
    ...
    def setup_menus(self):
        ...
        history_action.triggered.connect(self.show_history_manager)
    ...
    def show_bookmark_manager(self):
        ...
        table.setRowCount(len(self.config.BOOKMARKS))
        for row, (title, url) in enumerate(self.config.BOOKMARKS):
            table.setItem(row, 0, QTableWidgetItem(title))
            table.setItem(row, 1, QTableWidgetItem(url))
        
        btn_box = QDialogButtonBox()
        add_btn = btn_box.addButton("Add", QDialogButtonBox.ActionRole)
        add_btn.clicked.connect(lambda: self.add_bookmark_from_manager(table))
        
        remove_btn = btn_box.addButton("Remove", QDialogButtonBox.ActionRole)
        remove_btn.clicked.connect(lambda: self.remove_bookmark_from_manager(table))
        
        open_btn = btn_box.addButton("Open", QDialogButtonBox.ActionRole)
        open_btn.clicked.connect(lambda: self.open_bookmark_from_manager(table))
        
        btn_box.addButton(QDialogButtonBox.Save)
        btn_box.accepted.connect(lambda: self.save_bookmarks_from_manager(table, dialog))
    ...
    def add_bookmark_from_manager(self, table):
        row = table.rowCount()
        table.insertRow(row)
        table.setItem(row, 0, QTableWidgetItem("New Bookmark"))
        table.setItem(row, 1, QTableWidgetItem("https://"))

    def remove_bookmark_from_manager(self, table):
        selected = table.currentRow()
        if selected >= 0:
            table.removeRow(selected)

    def open_bookmark_from_manager(self, table):
        selected = table.currentRow()
        if selected >= 0:
            url = table.item(selected, 1).text()
            self.new_tab(url)

    def save_bookmarks_from_manager(self, table, dialog):
        self.config.BOOKMARKS = []
        for row in range(table.rowCount()):
            title = table.item(row, 0).text()
            url = table.item(row, 1).text()
            if title and url:
                self.config.BOOKMARKS.append((title, url))
        self.config.save_bookmarks(self.config.CURRENT_PROFILE)
        self.update_bookmarks_menu()
        dialog.accept()
    ...
    def add_to_history(self, url, title):
        ...
        history = history[:100]
        self.config.save_history(self.config.CURRENT_PROFILE)

    def show_history_manager(self):
        dialog = QDialog(self)
        dialog.setWindowTitle("History & Logs")
        ...
        history = self.config.load_history(self.config.CURRENT_PROFILE)
        for item in history:
            self.history_list.addItem(f"{item['time']} - {item['title']} ({item['url']})")
        
        history_btn_box = QDialogButtonBox()
        open_btn = history_btn_box.addButton("Open", QDialogButtonBox.ActionRole)
        open_btn.clicked.connect(self.open_history_item)
        
        remove_btn = history_btn_box.addButton("Remove", QDialogButtonBox.ActionRole)
        remove_btn.clicked.connect(self.remove_history_item)
        
        clear_btn = history_btn_box.addButton("Clear All", QDialogButtonBox.ActionRole)
        clear_btn.clicked.connect(self.clear_history)
        
        history_layout.addWidget(self.history_list)
        history_layout.addWidget(history_btn_box)
        ...
        logs_btn_box = QDialogButtonBox()
        refresh_btn = logs_btn_box.addButton("Refresh", QDialogButtonBox.ActionRole)
        refresh_btn.clicked.connect(self.refresh_logs)
        
        clear_logs_btn = logs_btn_box.addButton("Clear Logs", QDialogButtonBox.ActionRole)
        clear_logs_btn.clicked.connect(self.clear_logs)
        
        logs_layout.addWidget(self.logs_text)
        logs_layout.addWidget(logs_btn_box)
    ...
    def open_history_item(self):
        ...
        if selected >= 0:
            ...
            if selected < len(history):
                self.new_tab(history[selected]["url"])
    ...
    def remove_history_item(self):
        ...
        if selected >= 0:
            history = self.config.load_history(self.config.CURRENT_PROFILE)
            if selected < len(history):
                history.pop(selected)
                self.config.save_history(self.config.CURRENT_PROFILE)
                self.history_list.takeItem(selected)
    ...
    def show_settings(self):
        ...
        bg_layout.addWidget(bg_path_edit)
        bg_layout.addWidget(bg_browse_btn)
        bg_layout.addWidget(QLabel("Opacity:"))
        bg_layout.addWidget(opacity_slider)
        bg_layout.addWidget(opacity_label)
        bg_layout.addWidget(QLabel("Custom CSS:"))
        bg_layout.addWidget(css_edit)
